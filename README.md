...
# Внимание: данный скрипт не подключит ВМ в качестве участника кластера, а только установит нужные пакеты. Чтобы стать участником кластера, необходимо успешно подключиться по впн и выполнить join по токену и конфигу, полученному от мастер-ноды кластера
...
3) (Ok) Протестировать работу скриптов на серверной убунту 24.04
4) (WiP) Протестировать работу скриптов на десктоп убунту 24.04
...
2) Перейти в роль суперюзера, поменять тип файла
```
sudo su
...
4) В окне конфига (самое начало файла) поменять IP, name, password (пример приведен ниже):
